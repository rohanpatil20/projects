...
public class IEEE754Single {
	...
	public static String binRep(float d) {
		...
		String floatIEEE = String.format("%32s", Integer.toBinaryString(l)).replace(" ", "0");
		
		String sSign = floatIEEE.substring(0, 1);
		String sExponent = floatIEEE.substring(1, 9);
		String sMantissa = floatIEEE.substring(9);
		String sMantissaDecimal = "";
		
		int sign = Integer.parseInt(sSign);
		int exponent = Integer.parseInt(sExponent, 2); 
		int mantissa = 0;
		
		if (exponent==0 && sMantissa != "00000000000000000000000") {
			sMantissaDecimal = "0."+sMantissa;
		} else {
			sMantissaDecimal = "1."+sMantissa;
		}
		String s = sSign + "[" + sExponent + "]" + sMantissa;
		String t = (sign == 0) ? "+" : "-";

		int trueExponent = 0;
		if (exponent != 0 & sMantissaDecimal.substring(0,1) != "0") {
			trueExponent = exponent - Float.MAX_EXPONENT;
		} else if (exponent==0) {
			if (sMantissa != "00000000000000000000000") {
				trueExponent = exponent + Float.MIN_EXPONENT;
			}
		}
		if (d==0.0f) {
			trueExponent = 0;
		}
		t = t + sMantissaDecimal + "x2^(" + trueExponent + ")";
		return (s + "=" + t + "=" + d);
	}
}
